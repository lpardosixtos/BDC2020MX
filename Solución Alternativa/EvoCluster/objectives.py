...
def TWCV_vol(startpts, points, k, vols, gamma, gamma_nodes):
    labelsPred = getLabelsPred(startpts, points, k)
    ...
    if numpy.unique(labelsPred).size < k:
        ...
        nodes_fit = 0
        for i in range(k):
            curr_lab = numpy.where(labelsPred==i)
            nodes_fit += numpy.abs(604 - numpy.sum(curr_lab))
            vol_fit += numpy.abs((7973-sum(vols[curr_lab])))
        fitness += gamma*vol_fit/k
        fitness += gamma_nodes*nodes_fit/6
