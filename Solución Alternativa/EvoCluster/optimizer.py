...
def run(optimizer, objectivefunc, dataset_List, NumOfRuns, params, export_flags, 
	...
	vols=None, gamma=None, gamma_nodes=None):
	    ...
	    for j in range (0, len(objectivefunc)):
             for h in range(len(dataset_List)):
                ...
                for z in range (0,NumOfRuns):
                    ...
                    x=selector(optimizer[i],objective_name, k[h], f[h], PopulationSize,Iterations, points[h], metric, vols, gamma, gamma_nodes)
...
def selector(algo,objective_name, k, f, popSize,Iter, points, metric, vols=None, gamma=None, gamma_nodes=None):
	"""
	...
	if(algo=="FFA"):
		x=cffa.FFA(getattr(objectives, objective_name),lb,ub,k * f,popSize,Iter, k, points, metric, vols, gamma, gamma_nodes)        
	if(algo=="GWO"):
